...
* `browser.tabs.insertAfterCurrent` / `browser.tabs.insertAfterCurrentExceptPinned`
...
* `network.predictor.enable-hover-on-ssl`
  ...
  + ublock origin disables prefetching entirely for privacy reasons, so you'd have to disable that option in `settings > privacy > uncheck "disable pre-fetching"`
